...
export function KioskItems({ kioskId, kioskOwnerCap, address }: { address: string } & LocalKioskDataParams): JSX.Element {

    const provider = useRpc();

    const [loading, setLoading] = useState<boolean>(false);

    const [kioskData, setKioskData] = useState<KioskData>({ items: [], listings: [], itemIds: [], listingIds: [] });
    const [modalItem, setModalItem] = useState<OwnedObjectType | null>(null);
    const [kioskItems, setKioskItems] = useState<OwnedObjectType[]>([]);

    const [kioskListings, setKioskListings] = useState<Record<string, KioskListingValue>>()
