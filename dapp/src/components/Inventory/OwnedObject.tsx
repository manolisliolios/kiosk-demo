...
export function OwnedObject({ object, placeFn, listFn }: KioskData & { listFn: (item: OwnedObjectType) => void, placeFn: (item: OwnedObjectType) => void; object: OwnedObjectType }): JSX.Element {
    ...
    return (
        <DisplayObject item={object}>
            <>
                <Button  loading={loading} onClick={() => actionWithLoader(placeFn, object, setLoading)}>Place in kiosk</Button>
                <Button loading={loading} onClick={()=> actionWithLoader(listFn, object, setLoading)} className="btn-outline-primary">List in Kiosk</Button>
            </>
        </DisplayObject>
    )
}
